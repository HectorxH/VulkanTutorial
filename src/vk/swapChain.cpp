...
vk::SwapChainSupportDetails::SwapChainSupportDetails(
    VkPhysicalDevice device, std::shared_ptr<vk::Surface> surface
) {
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      device, surface->surface, &capabilities
  );
  ...
  if (formatCount != 0) {
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(
        device, surface->surface, &formatCount, formats.data()
    );
  }

  uint32_t presentModeCount;
  vkGetPhysicalDeviceSurfacePresentModesKHR(
      device, surface->surface, &presentModeCount, nullptr
  );

  if (presentModeCount != 0) {
    presentModes.resize(presentModeCount);
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        device, surface->surface, &presentModeCount, presentModes.data()
    );
  }
}
